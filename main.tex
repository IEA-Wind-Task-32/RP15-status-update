...
\begin{document}
	...
	in 2013 IEA Wind Task 32 and Task 11 published "Recommended Practices ground-based remote sensing for wind resource assessment" \cite{RP15_2013}. This was published in response to a need from the wind energy community for a document that would establish repeatable good practice in measurements with active remote sensing devices (RSDs), specifically wind lidar and sodar.
	
	Since 2013, IEA Wind Task 32 and others have developed and codified new understanding of how best to use wind lidar. As a result, it is no longer clear what the user should turn to.
	
	\subsection*{Our goal}
	The goal of this update is the same as was stated in the preface of RP15:
	
	\begin{shadequote}[r]{RP15 (2013)}
... to document the steps required to collect high-quality, well-documented remote sensing data for use in wind resource assessments on land.
	...
	The use case considered here is the same as was stated in the preface of RP15, but limited to wind lidar:

	\begin{shadequote}[r]{RP15 (2013)}
	... ground-based, fixed scan geometry, vertically-profiling wind remote sensing using lidar ... for the resource assessment phase of an on-shore wind farm development. Lidar ... that use these methods are referred to generically in the document as remote sensing devices, or RSD.''
	\end{shadequote}
	
	\subsection*{Relevant documents}
	
	The \href{https://github.com/IEA-Wind-Task-32/RP15-Ground-Based-Remote-Sensing-for-Wind-Resource-Assessment/releases/tag/1.0}{2013 IEA Wind Recommended Practices document} (Fig \ref{fig:RP15_2013}) provided specific guidance as ``Recommended Practices'' (e.g., \RP{1}) and informative notes (e.g. Note 1) about how to use remote sensing. These covered many different aspects of the remote sensing lifecycle. These RPs were not always based on evidence, and so were called ``recommended practices'', not a standard.
	...
	The following section discusses the current documents that are applicable to the lifecycle of a wind lidar deployment. They use the order established in the 2013 IEA Wind Recommended Practices:
	
	\begin{\begin{itemize}
		\item Characterising remote sensing devices ()
	\end{itemize}}
	...
	A remote sensing device should be appropriately characterized. The important characteristics of a wind lidar are described in RP15.

	$\rightarrow$ RP15 \RPdetails{1}{Documentation of RSD characteristics}
