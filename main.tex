...
In 2013 IEA Wind Task 32 and Task 11 published "Recommended Practices ground-based remote sensing for wind resource assessment" \cite{RP15_2013}, or \IEARP. It was published to satisfy a need from the wind energy community for documented good practice in measurements with active remote sensing devices (RSDs), including lidar and sodar.
...
\todo[inline]{Do we prefer IEC 61400-12-1 (2017) or MEASNET as the indicator of complex terrain?}
...
An IEA Wind Task 32 working group on the use of wind lidar in complex terrain is carrying out a group study on several different transfer methods. Results are expected in 2021 and the experience gained by the group may lead to the publication of guidelines for this use case.
...
The user should check that the information in the installation report satisfies their stakeholders and considers future needs. Many vendors and third-party engineers provide templates to help collect this data. Furthermore, \IEARPndetails{16}{Installation report} also provides a suggested list of things to include in an installation report.
...
\begin{itemize}
  \item Most lidars are checked by the manufacturer before delivery to customers. This can take the form of a short measurement campaign where the lidar measures winds near a reference anemometer mounted on a tall tower. A comparison of the results shows if the device is performing as expected. This comparison could be carried out according to the IEC 61400-12-1 (2017) standard \cite{IEC_61400_12_1_2017}. This is called `unit certification'. Alternatively, the lidar can be placed next to a lidar that is regularly compared to a cup anemometer on a mast. This approach is called `type certification'. These pre-delivery tests are identical to those offered for cup anemometers. Users may therefore find pre-delivery testing of lidar is  sufficient for their needs.
...
\todo[inline]{Added the first bullet above re. manufacturer's testing. Not sure I've got the right tone yet?}

\section*{Summary}
Many groups have published documents that offer guidance about the use of wind lidar for wind resource assessment.

Many of the recommendations made in the 2013 \IEARP\ are still applicable in 2020, particularly in relation to general good practice when deploying, operating, and maintaining wind lidar.
...
Upcoming new IEC standards for resource assessment, instrumentation and measurements -- to be published in the 2020's -- may in turn deprecate these documents. Similarly, community and industry initiatives (by Task 32, CFARS, and DNV-GL) may lead to convergence on methods.
...
We anticipate revising this document again during the period 2021-2023 to reflect feedback and new developments. Readers are encouraged to check \href{\conceptDOI}{\conceptDOI} for revisions.
